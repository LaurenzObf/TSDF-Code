...
def run_one(
    deformation_type: str, depth: int, value: int, run_root: Path
) -> Dict[str, object]:
    t0 = time.time()
    ...
    d_m = scene.compute_distance(q).numpy()
    ist_defo_mm = (1000.0 * d_m).astype(np.float32)
    ...
    print(
        ...
        f"thr={threshold:.6f}, runtime={runtime_s:.2f}s, LAS={las_path.name}"
    )
    return {
        "run_name": run_name,
