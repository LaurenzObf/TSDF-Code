...
- `bulge_pointcloud.py`: Erzeugt lokale Beulen-Deformationen und schreibt `soll_defo_mm`.
- `translate_scan_zComponent.py`: Erzeugt z-Translationen und schreibt `soll_defo_mm`.
- `Sweeper_tsdf_deformation.py`: TSDF-Sweeper fuer `beule`, `tz` und `tilt` mit LAS-Export (`tsdf_value_mm`).
